...
class Minimap {
    ...
    create(scene) {
        ...
        // 바다 평면 생성 (미니맵 배경)
        const seaGeometry = new THREE.PlaneGeometry(this.options.size * 30, this.options.size * 30);
    ...
    update(time) {
        ...
        this.entities.forEach(item => {
            if (item.entity && item.entity.mesh && item.marker) {
                ...
                // 마커 방향 업데이트 (선박의 방향에 맞춤)
                if (typeof item.entity.direction !== 'undefined') {
                    item.marker.rotation.y = item.entity.direction;
                }
